...
CREATE OR REPLACE VIEW ventas_cruzadas(ID_EMPLEADO, VENTA_CRUZADA)  AS  select `venta`.`ID_EMPLEADO` AS `ID_EMPLEADO`,avg(`venta`.`TOTAL_ARTICULOS`) AS `VENTA_CRUZADA` from `venta` group by `venta`.`ID_EMPLEADO` having (avg(`venta`.`TOTAL_ARTICULOS`) >= 1.6) ;

--
-- Estructura para la vista `buenas_ventas`
--

CREATE OR REPLACE VIEW buenas_ventas(ID_EMPLEADO, NUMERO_VENTAS)  AS  select `venta`.`ID_EMPLEADO` AS `ID_EMPLEADO`,count(0) AS `NUMERO_VENTAS` from `venta` where (`venta`.`TOTAL_VENTA` >= 50) group by `venta`.`ID_EMPLEADO` ;

--
-- Estructura para la vista `top_articulos`
--

CREATE OR REPLACE VIEW top_articulos(ID_ARTICULO, CANTIDAD)  AS  select `info_venta`.`ID_ARTICULO` AS `ID_ARTICULO`,sum(`info_venta`.`CANTIDAD`) AS `CANTIDAD` from `info_venta` group by `info_venta`.`ID_ARTICULO` order by sum(`info_venta`.`CANTIDAD`) desc limit 5;

--
-- Estructura para la vista `top_articulo_empleado`
--

CREATE OR REPLACE VIEW top_articulo_empleado(ID_EMPLEADO, ID_ARTICULO)  AS  select `venta`.`ID_EMPLEADO` AS `ID_EMPLEADO`,`info_venta`.`ID_ARTICULO` AS `ID_ARTICULO`,sum(`info_venta`.`CANTIDAD`) AS `CANTIDAD` from (`venta` join `info_venta`) where  (`venta`.`ID_VENTA` = `info_venta`.`ID_VENTA`) group by `info_venta`.`ID_ARTICULO` order by sum(`info_venta`.`CANTIDAD`) desc limit 1 ;

--
-- Estructura para la vista `sueldos_rentables`
--

CREATE OR REPLACE VIEW sueldos_rentables(ID_EMPLEADO, SUELDO)  AS  select `venta`.`ID_EMPLEADO` AS `ID_EMPLEADO`,`empleado`.`SUELDO` AS `SUELDO` from (`venta` join `empleado`) where ((`venta`.`ID_EMPLEADO` = `empleado`.`ID`)) group by `venta`.`ID_EMPLEADO` having (sum(`venta`.`TOTAL_VENTA`) <= (`empleado`.`SUELDO` * 5)) ;

--
-- Estructura para la vista `empleado_en_top`
--

CREATE OR REPLACE VIEW empleado_en_top(ID_EMPLEADO)  AS  select `top_articulo_empleado`.`ID_EMPLEADO` AS `ID_EMPLEADO` from `top_articulo_empleado`,`top_articulos` where (`top_articulo_empleado`.`ID_ARTICULO` = `top_articulos`.`ID_ARTICULO`) ;
